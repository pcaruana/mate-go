...
func main() {
	...
	doej := &user{"Joe Doe", "doej@example.com"}
	doej.notify()
	doej.changeEmail("doej@gmail.com")

	// doej in this example is a pointer that has the type *user. We are still able to call notify.
	// This is still correct. As long as we deal with the type user, Go can adjust to make the call.

	// Behind the scene, we have something like (*doej).notify(). Go will take the value that doej 
