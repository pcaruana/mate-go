...
import (
	...
	"github.com/pcaruana/work-mate-go/go/testing/web_server/handlers"
)
...
func TestSendJSON(t *testing.T) {
	...
	t.Log("Given the need to test the SendJSON endpoint.")
	{
		...
		t.Logf("\tTest 0:\tWhen checking %q for status code %d", url, statusCode)
		{
			...
			if u.Name == "Joe Doe" {
				t.Logf("\t%s\tShould have \"Joe Doe\" for Name in the response.", succeed)
			} else {
				t.Errorf("\t%s\tShould have \"Joe Doe\" for Name in the response : %q", failed, u.Name)
			}

			if u.Email == "doej@example.com" {
				t.Logf("\t%s\tShould have \"doej@example.com\" for Email in the response.", succeed)
			} else {
				t.Errorf("\t%s\tShould have \"doej@example.com\" for Email in the response : %q", failed, u.Email)
			}
		}
	}
}
